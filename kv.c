...
int createItem(const char* key, char* value) {
    ...
    char* copy1 = malloc(strlen(value) + 1);
    if (copy == NULL || copy1==NULL) { return -1; }
    strncpy(copy, key, strlen(key) + 1);
    strncpy(copy1, value, strlen(value) + 1);
    items[nItems].key = copy;
    items[nItems].value = copy1;
    nItems++;
    return 0;
}
...
int updateItem(const char* key, char* newValue) {
    ...
    struct item *i = findItem(key);
    if (i == NULL) { return -1; }
    char* copy1 = malloc(strlen(newValue) + 1);
    strncpy(copy1, newValue, strlen(newValue) + 1);
    i->value = copy1;
    return 0;
}
